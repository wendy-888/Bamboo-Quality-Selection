...
f = open("healthy_bamboo_location.txt","w")
...
#           print fix status
            ...
            f.write('Latitude(%c),Longitude(%c),%.9f,-%.9f'%(parser.latitude[1],parser.longitude[1],parser.latitude[0],parser.longitude[0]))
            f.write('\n')
            f.write('UTC Timestamp:%d:%d:%d'%(parser.timestamp[0],parser.timestamp[1],parser.timestamp[2]))
            f.write('\n')
